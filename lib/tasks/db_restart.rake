...
namespace :db do
  ...
  task :restart_dev do
    ...
    system('bundle exec rake db:environment:set db:drop db:create db:migrate db:seed APP_ENV=development VERBOSE=true')
  ...
  task :restart_test do
    # rubocop:disable Layout/LineLength
    system('bundle exec rake db:environment:set db:drop db:create db:migrate db:seed APP_ENV=test VERBOSE=true')
    # rubocop:enable Layout/LineLength
  end
