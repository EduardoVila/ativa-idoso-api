...
require 'spec_helper'
require 'helpers/pro_score/presumed_income_helpers'
require_relative '../integrable'

RSpec.configure do |c|
  c.include PresumedIncomeHelpers, :presumed_income_helpers
end

RSpec.describe ProScore::PresumedIncomeIntegrator,
               :presumed_income_helpers do
    ...
    context 'when it is success' do
      ...
      it 'creates the report correctly' do
        ...
        expect { subject }.to change(ProScore::Report, :count).by(1)
          .and change(ProScore::ProprableProfession, :count).by(1)
          .and change(ProScore::PresumedSalaryRange, :count).by(1)
          .and change(ProScore::PresumedIncome, :count).by(1)
      end
    end
