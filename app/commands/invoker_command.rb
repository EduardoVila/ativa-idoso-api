...
# InvokerCommand: interface of the command pattern and the command invoker.
#
# The workflow of the alpop-analysis project is based on the command pattern.
#
# The InvokerCommand class is responsible for executing various commands
...
# - :analysis_webhook_trigger_command -> API::WebhookTriggerCommand
# - :boa_vista_cadastral_command -> BoaVista::CadastralCommand
# - :step_command -> Analysis::StepCommand
...
#   - arg: Argument to be passed to the command.
...
class InvokerCommand
  COMMANDS = {
    analysis_report_runner_command: 'Analysis::ReportRunnerCommand',
    analysis_item_runner_command: 'Analysis::ItemRunnerCommand',
    webhook_trigger_command: 'API::WebhookTriggerCommand',
    boa_vista_cadastral_command: 'BoaVista::CadastralCommand',
  ...
  def execute(command, arg, klass_name = nil)
    if COMMANDS.key?(command.to_sym)
      COMMANDS[command.to_sym].constantize.call(arg)
    elsif command == :a_step
      klass_name.constantize.call(arg)
    else
